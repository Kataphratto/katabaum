...
extract( $atts );
$el_class .= ' ' . (!empty($tab_layout) ? $tab_layout : 'style_1');
...
$output .= (!empty($short_desc)) ? '<div class="widget-sub-title">' . nl2br(esc_html($short_desc)) . '</div>' : '';
...
if(!empty($left_banner)){
	$left_image = wp_get_attachment_image_src($left_banner, 'full');
	$output .= '<div class="vc_tta-left-banner col-md-3 visible-lg visible-md"><img alt="" src="'. $left_image[0] .'" alt=""/></div>';
	$output .= '<div class="vc_tta-panels col-xs-12 col-md-9">';
}else{
